//! Installed apps, matched by spoken name and launched on request.
//!
//! macOS lists `.app` bundles in the usual folders and opens them with
//! `open -a`. Linux reads XDG desktop entries and runs their `Exec` line.
...
pub struct App {
    pub name: String,
    /// A desktop entry's `GenericName`, like "Web Browser".
    generic: Option<String>,
    /// The `.app` bundle on macOS, the `.desktop` file on Linux.
    path: PathBuf,
...
/// Every app the user could mean, sorted by name.
...
fn collect_bundles(dir: &Path, depth: usize, apps: &mut Vec<App>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.extension().is_some_and(|ext| ext == "app") {
            ...
            // Suites such as "Microsoft Office" keep their apps one level down.
...
/// Where desktop entries live, most important first: a user's own entry
/// hides the system one with the same id.
#[cfg(not(target_os = "macos"))]
...
/// The app a desktop entry describes, unless it is hidden or not an app.
#[cfg_attr(target_os = "macos", allow(dead_code))]
...
/// The string escapes every desktop entry value may use.
#[cfg_attr(target_os = "macos", allow(dead_code))]
...
/// Splits an `Exec` line into a program and its arguments, dropping the
/// file and URL placeholders a launcher would fill in.
#[cfg_attr(target_os = "macos", allow(dead_code))]
...
fn score(app: &App, query: &str) -> Option<u8> {
    ...
    // "Chrome" for "Google Chrome", "Word" for "Microsoft Word".
    ...
    // One slip in a long name, as speech models make ("Spotfy").
...
/// Common spoken names that are not the app's real name.
...
/// Lowercase letters and digits only, so "Visual Studio Code" and
