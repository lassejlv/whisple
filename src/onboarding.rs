...
use gpui_kit::{
    div, prelude::*, px, size, App, AppContext, Bounds, BoxShadow, Context, Div, FocusHandle,
    Focusable, FontWeight, IntoElement, Render, SharedString, Styled, Window,
...
impl Onboarding {
    fn new(window: &Window, cx: &mut Context<Self>) -> Self {
        let selected = models::spec(&settings::load().selected)
    ...
    fn finish(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        prefs.selected = self.selected.id.to_string();
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child("No account needed · runs on your Mac"),
                    ),
            )
    }
        ...
        div()
            ...
            .child(
                div()
                    .text_size(px(12.0))
                    .text_color(theme::TERTIARY)
                    .child("Three more models live in the picker, from 142 MB to 1.5 GB."),
            )
            .when_some(self.error.as_ref(), |this, error| {
                this.child(
                    div()
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .font_weight(FontWeight::SEMIBOLD)
                    .text_color(theme::AMBER)
                    .child(format!("✓  {} is ready", self.selected.name)),
            )
            .child(
                div()
                    .flex()
