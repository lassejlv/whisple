//! The trial server at cloud.whisple.app remembers when each Mac's free trial
//! started, so deleting Whisple's local copies cannot start a new one.
//!
//! Whisple sends a hash of the hardware ID and gets back a token signed with
//! the server's Ed25519 key. The token is kept beside the local trial and
//! checked offline; only this Mac's token with a valid signature counts.
...
/// What a verified token says about this Mac's trial.
...
/// Reads `"IOPlatformUUID" = "…"` from `ioreg` output.
...
/// Asks the server for this Mac's trial token.
...
pub(crate) mod tests {
    ...
    /// A token as the server builds it.
    ...
    fn the_device_id_is_a_hash_of_the_hardware_id() {
        // python3 -c 'import hashlib; print(hashlib.sha256(b"whisple-trial-v1:ABC-123").hexdigest())'
        assert_eq!(
            device_hash("ABC-123"),
    ...
    fn a_signed_token_for_this_mac_gives_its_trial() {
        ...
        // Padded base64url reads the same.
        ...
        // An old key alongside the new one still accepts the token.
    ...
    fn a_forged_copied_or_damaged_token_is_refused() {
        ...
        // A later start written into the payload breaks the signature.
        ...
        // An unknown version or nonsense.
