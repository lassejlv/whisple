...
mod tests {
    ...
    fn stored_shortcuts_map_to_native_keys_and_modifiers() {
        for (source, code, modifiers) in [
            ...
            let chord = crate::platform::hotkey::parse(source).unwrap();
            assert_eq!(
    ...
    fn invalid_shortcuts_cannot_replace_a_registration() {
        assert!(native_shortcut(&crate::platform::hotkey::parse("k").unwrap()).is_err());
        let mut chord = crate::platform::hotkey::parse("ctrl-space").unwrap();
