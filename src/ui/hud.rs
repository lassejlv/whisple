...
use crate::transcription::cloud::{self, GatewayModel, Provider};
use crate::ui::motion;
...
impl Whisp {
    ...
    fn bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let hint = match &self.phase {
            ...
            _ => Some({
                #[cfg(feature = "licensing")]
                if let Some(left) = self.trial_ending() {
                    hint_text(tf("Trial · {}", &[&license::trial_left(left, true)]))
                        .text_color(theme::AMBER)
                } else {
                    hint_text(hotkey::symbols(if self.record_hotkey.is_empty() {
                        &self.show_hotkey
                    } else {
                        &self.record_hotkey
                    }))
    ...
    #[cfg(not(feature = "licensing"))]
    fn unlock_capsule(&self, _buy: bool, cx: &mut Context<Self>) -> impl IntoElement {
        self.model_capsule(cx)
    }
    ...
    fn voice_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        press_handlers(
            div()
                .id("voice")
                .role(gpui_kit::Role::Button)
                .aria_label(if matches!(self.phase, Phase::Listening(_)) {
                    t("Stop recording")
                } else {
                    t("Start recording")
                })
    ...
    fn copy_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        press_handlers(
            div()
                .id("copy")
                .role(gpui_kit::Role::Button)
                .aria_label(if copied { t("Copied") } else { t("Copy") })
                .h(px(28.0))
                .flex()
                .items_center()
                .justify_center(),
