...
impl SettingsWindow {
    pub(in crate::ui::settings) fn toggle_menu_bar(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::ui::settings) fn toggle_startup(&mut self, cx: &mut Context<Self>) {
        self.hud
            .update(cx, |hud, cx| hud.toggle_open_on_startup(cx));
        cx.notify();
    }

    pub(in crate::ui::settings) fn toggle_copy(&mut self, cx: &mut Context<Self>) {
        self.hud.update(cx, |hud, cx| hud.toggle_copy_notes(cx));
        cx.notify();
    }

    pub(in crate::ui::settings) fn toggle_clean(&mut self, cx: &mut Context<Self>) {
        self.hud.update(cx, |hud, cx| hud.toggle_clean_fillers(cx));
        cx.notify();
    }

    pub(in crate::ui::settings) fn toggle_commands(&mut self, cx: &mut Context<Self>) {
        self.hud.update(cx, |hud, cx| hud.toggle_voice_commands(cx));
        cx.notify();
    }

    pub(in crate::ui::settings) fn toggle_screen(&mut self, cx: &mut Context<Self>) {
        self.hud.update(cx, |hud, cx| hud.toggle_screen_context(cx));
        cx.notify();
    }

    pub(in crate::ui::settings) fn begin_shortcut(
        &mut self,
        slot: Shortcut,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::ui::settings) fn general(&self, cx: &mut Context<Self>) -> Div {
