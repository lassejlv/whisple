...
impl SettingsWindow {
    pub(in crate::ui::settings) fn start_talking(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::ui::settings) fn activate_license(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::ui::settings) fn refresh_license(&mut self, cx: &mut Context<Self>) {
        if self.license_busy {
            return;
        }
        self.license_busy = true;
    ...
    pub(in crate::ui::settings) fn deactivate_license(&mut self, cx: &mut Context<Self>) {
        if self.license_busy {
            return;
        }
        self.license_busy = true;
    ...
    pub(in crate::ui::settings) fn license(&self, cx: &mut Context<Self>) -> Div {
