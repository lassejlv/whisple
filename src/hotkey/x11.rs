...
struct Grab {
    /// The chord for each `Shortcut`, by index.
...
/// One bit per `Shortcut` index, set by the listener thread.
...
const MOD2: u16 = 16;
const MOD4: u16 = 64;
/// Modifiers that tell chords apart. Caps Lock and Num Lock are ignored.
const RELEVANT: u16 = SHIFT | CONTROL | MOD1 | MOD4;

/// A grabbed shortcut: its keycode and chord.
...
/// The grabbed shortcut a key event belongs to.
...
mod tests {
    ...
    fn two_shortcuts_on_one_key_are_told_apart() {
        let show = crate::hotkey::parse("ctrl-shift-space").unwrap();
        let record = crate::hotkey::parse("ctrl-alt-space").unwrap();
        ...
        // Caps Lock and Num Lock do not change which shortcut was pressed.
        assert_eq!(
