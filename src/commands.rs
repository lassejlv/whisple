//! Spoken commands: "Open Spotify", "Go to github.com", and requests
//! addressed to the assistant, like "Hey Whisple, what does this error mean?"
//!
//! Parsing only looks at the words. Whether "Open Notes" is a command or a
//! note to type depends on an app called Notes being installed, which the
//! caller checks with `apps::find`.
...
pub enum Command {
    /// An app name as spoken, still to be matched against installed apps.
    OpenApp(String),
    /// A full `https://` URL.
    OpenUrl(String),
}
...
/// Parses a whole transcript as a command, or `None` for dictation.
...
/// The request after a wake phrase such as "Hey Whisple," or `None` when the
/// transcript is not addressed to the assistant.
...
/// Lowercase words with the punctuation Whisper adds around a sentence
/// removed, keeping dots inside words so "github.com" survives.
...
/// `text` without the leading phrase `prefix`, when it is followed by a word
/// boundary.
...
/// Drops articles and filler around the thing to open: "the Spotify app for
/// me" becomes "spotify".
...
/// A spoken site, like "github.com" or "github dot com", as a URL.
...
/// Recovers the capitalization the transcript used for `target`, so the
/// result reads "Opened Visual Studio Code" rather than lowercase.
