//! Rows, switches and sections shared by the settings pages.
...
/// A section whose label carries a quiet note on the right, such as disk use.
...
/// A dialog button: amber for the main action, raised for the rest.
...
impl SettingsWindow {
    ...
    pub(super) fn switch(&self, id: &'static str, on: bool) -> AnyElement {
        ...
        let track = div()
            ...
            .rounded_full()
            .bg(theme::TRACK);
        // How far toward "on" the switch is at `t` of the transition.
