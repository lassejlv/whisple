//! What the user is looking at, for the assistant: the front app, its window
//! title, selected text, and a screenshot.
//!
//! `focused` runs as the bar opens, before Whisple takes focus, and must stay
//! quick. The screenshot is only taken when a request is actually sent.

/// Selected text beyond this is cut; the screenshot shows the rest.
const MAX_SELECTION: usize = 4_000;
/// Screenshots are scaled to at most this width before upload.
...
/// The app and window the user was in, captured before the bar takes focus.
...
/// A PNG of the main display. Slow; call it off the UI thread.
...
mod macos {
    ...
    pub fn focused() -> Snapshot {
        ...
        // Window titles and selections need Accessibility access, which
        // typing into other apps already asks for.
    ...
    pub fn capture_png() -> Result<Vec<u8>, String> {
        ...
        // Retina captures are twice the size a model needs to read them.
...
mod x11 {
    ...
    /// The window or nearest ancestor that carries a title. Focus often sits
    /// on a child of the client window.
    ...
    /// WM_CLASS holds "instance\0Class\0"; the class reads like an app name.
    ...
    /// The PRIMARY selection: whatever text is highlighted in another app.
    ...
    /// Packed RGB from 32-bit X pixels, which are BGRX in little-endian order.
    pub(super) fn to_rgb(
        data: &[u8],
        width: u32,
        height: u32,
...
mod tests {
    ...
    fn wide_screens_are_averaged_down() {
        // Four pixels wide: black, white, red, blue. Half width averages pairs.
    ...
    fn x_pixels_become_rgb() {
        // Two BGRX pixels in a row padded to 12 bytes.
